...
function least_squares(ydata, f::Function, g::Function, p0, wt, method::Optimizer; kwargs...)
    results = method(f, g, p0; kwargs...)
    p = minimizer(results)
    resid = f(p)
    n = length(resid)
    dof = n - length(p)
...
function least_squares(ydata, f::Function, p0, wt; kwargs...)
    ...
    least_squares(ydata, f, g, p0, wt; kwargs...)
end
...
function curve_fit(model::Function, xpts::AbstractArray, ydata::AbstractArray, p0; kwargs...)
    ...
    T = eltype(ydata)
    least_squares(ydata, f,p0,T[]; kwargs...)
end
...
function curve_fit(model::Function, jacobian_model::Function,
    ...
    T = eltype(ydata)
    least_squares(ydata, f, g, p0, T[]; kwargs...)
end
...
function curve_fit(model::Function, xpts::AbstractArray, ydata::AbstractArray, wt::Vector, p0; kwargs...)
    ...
    least_squares(ydata, f,p0,wt; kwargs...)
end
